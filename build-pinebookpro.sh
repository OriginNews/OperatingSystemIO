...
export packages="origin-minimal origin-standard origin-desktop initramfs-tools linux-firmware"
...
version=6.0
YYYYMMDD="$(date +%Y%m%d)"
imagename=originos-$version-$channel-pinebookpro-$YYYYMMDD
...
echo 'CONFIG_IDENT_STRING=" origin ARM"' >> .config
...
# Working directory
work_dir="${basedir}/origin-${architecture}"
...
echo "origin" > "${work_dir}/etc/hostname"
